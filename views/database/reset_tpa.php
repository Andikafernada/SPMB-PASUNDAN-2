...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reset_single'])) {
    ...
    if (verify_csrf_token($csrf)) {
        ...
        if ($id_siswa > 0) {
            // Ambil nama siswa dulu untuk log
            ...
            // Log aktivitas (ke audit_log jika ada)
        ...
        <div class="p-6 md:p-8 lg:p-10 pb-4 flex-shrink-0 animate-fade-in">
            ...
            <div class="flex flex-col xl:flex-row gap-4 items-start xl:items-center justify-between">
                ...
                <div class="flex items-center gap-3 w-full xl:w-auto">
                    <select onchange="window.location.href='?status=<?= $filter_status ?>&search=<?= $search ?>&jurusan='+this.value" class="bg-white border border-slate-200 text-indigo-600 text-xs font-bold rounded-xl px-4 py-3 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 appearance-none cursor-pointer shadow-sm">
                    ...
                    <form method="GET" action="" class="relative flex-1 xl:w-64">
                        ...
                        <input type="hidden" name="jurusan" value="<?= $filter_jurusan ?>">
                    ...
                    <div class="divide-y divide-slate-100">
                        ...
                        <div class="grid grid-cols-[50px_2fr_1fr_1fr_1fr_1fr_140px] gap-4 px-4 py-3 items-center hover:bg-slate-50/80 transition-colors group rounded-xl">
                            ...
                            <div>
                                ...
                                <span class="inline-block mt-1 px-2 py-0.5 bg-purple-50 border border-purple-100 text-purple-600 text-[9px] font-black uppercase rounded">Kls <?= $s['kelas'] ?></span>
                                <?php endif; ?>
                            </div>

                            <div>
                                <span class="inline-block px-2.5 py-1 bg-indigo-50 border border-indigo-100 text-indigo-700 text-[9px] font-black uppercase tracking-wider rounded-md"><?= $s['jurusan'] ?></span>
                            </div>
                            ...
                            <div class="text-center">
                                <?php if($nilai_total !== '-'): ?>
                                    ...
                                    ?>
                                    <span class="inline-block px-3 py-1 <?= $color_class ?> text-xs font-black rounded-md"><?= $nilai_total ?></span>
                                <?php else: ?>
